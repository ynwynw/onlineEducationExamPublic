<template>
  <div>
    <el-card v-for="(question, index) in questionDataList" style="width: 100%" :key="index" class="box_card">
      <div slot="header" class="clearfix">
        <span style="font-weight: bold">{{question.questionTypeName}}</span>
      </div>
      <div class="text item">
        ...
        <el-row v-if="question.questionType === 6" class="option">
          <el-radio :disabled="true" v-model="question.studentAnswer" label="1">对</el-radio>
          <el-radio :disabled="true" v-model="question.studentAnswer" label="0">错</el-radio>
        </el-row>
        ...
        <div class="text item">
          答题时间:  {{question.createDate}}
        </div>

        <div class="text item">
          科目:  <el-tag type="success">{{question.subjectName}}</el-tag>
        </div>
            ...
            <div class="text item">
              <div class="demo-image__preview">
                <el-image
                  ...
                  :src="url"
                  :preview-src-list="question.studentAnswer"
                ></el-image>
              </div>
            </div>
          </template>
        ...
        <template v-else>
          <div v-if="question.studentAnswer" class="text item">
        ...
        <div style="border-bottom: 1px solid #EBEEF5" class="text item">
          <span @click="playVideo(question.videoUrl)" class="play" ><i class="el-icon-video-play"></i> 视频点评 </span>
        </div>
      </div>
    </el-card>
  </div>
...
<script>
  export default {
    ...
    watch: {
      questionList (val) {
        this.questionDataList = val
        if (this.questionDataList.length > 0) {
          this.parseQuestion()
        }
      }
    },
      ...
      parseQuestion () {
        ...
        this.questionDataList.forEach(item => {
            ...
            if (item.questionType === 1 || item.questionType === 2) {
              ...
              if (options) {
                item.optionList = JSON.parse(options)
              }
            }


            // 试题类型非单选和判断题， 解析答案数组
            if (item.questionType !== 1 && item.questionType !== 6 && item.questionType !== 8) {
                let studentAnswerStr = item.studentAnswer
                ...
                if (studentAnswerStr) {
                  studentAnswerStr.split(',').forEach(value => {
                    ...
                    if (value) {
                      studentAnswerArray.push(host + value)
                    }
                  })
                  item.studentAnswer = studentAnswerArray
                }
            }
        })
      }
