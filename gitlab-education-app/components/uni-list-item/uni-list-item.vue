				...
				<slot name="body">
					<view class="uni-list-item__content" :class="{ 'uni-list-item__content--center': thumb || showExtraIcon || showBadge || showSwitch }">
						...
						<text v-if="note" class="uni-list-item__content-note">
						  {{ note }}
						  <text style="margin-left: 50upx;">55人考过</text>
						</text>
					</view>
				</slot>
				<slot name="footer">
					...
					<!-- 	<button v-if="showType === 'button'" type="warn" size="mini">开始答题</button> -->
						<text v-if="showType === 'text'" class="uni-list-item__extra-text">{{ rightText }}</text>
